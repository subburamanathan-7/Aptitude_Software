...
import { sessionCheck,getSession } from '../features/users/UserServices';
function MainDashboard(props) {
    const navigate = useNavigate();
    const [responseStatus,setResponseStatus] = useState(Boolean(false))
    const [beginStatus,setBeginStatus] = useState(Boolean(false))
    ...
    const getSessionMutation = useMutation({
        ...
        onSuccess:(data)=>{
            ...
            console.log(data.currentSession['0'])
    ...
    const sessionCheckQuery = useQuery({
        ...
        queryFn:()=>{
            return sessionCheck(sessionStorage.getItem('user')) 
        },
        refetchOnMount:true,
    ...
    if(sessionCheckQuery.isLoading){
    ...
    else if(sessionCheckQuery.isFetched ){
        // sessionCheckQuery.data.currentSession

        let currentTime = new Date()
        let currentHours = currentTime.getHours();

        sessionStorage.setItem('endTime',sessionCheckQuery.data?.currentSession['0'].endTime)
        sessionStorage.setItem('startTime',sessionCheckQuery.data?.currentSession['0'].startTime)

        let endTime = sessionCheckQuery.data.currentSession['0'].endTime[0] 
        + sessionCheckQuery.data.currentSession['0'].endTime[1];

        let startTime = sessionCheckQuery.data.currentSession['0'].startTime[0] 
        + sessionCheckQuery.data.currentSession['0'].startTime[1];
        // console.log(endTime)
        if(Number(currentHours) <= Number(endTime) && Number(currentHours) >= Number(startTime)){
            if(beginStatus!==true){
                setBeginStatus(true)
            }
        }
        ...
        // if(beginStatus!==sessionCheckQuery.data.responseExists){
        //     setResponseStatus(sessionCheckQuery.data.responseExists)
        // }
    //    console.log(sessionCheckQuery.data.currentSession[0].endTime)
    //    console.log(sessionCheckQuery.data.currentSession[0].startTime)
    ...
    return (
        ...
        <div className='flex flex-wrap justify-center my-[2%]'>
            <div className='flex rounded-lg w-[90%] my-[5%] mx-[5%]'>
                <div className='p-[3%] rounded-lg shadow-md w-full max-w-screen '>
                    ...
                    <div className='text-left leading-8'>
                        ...
                        3.Do not indulge yourself in any malpractices.<br/>
                    ...
                    <div className='text-left leading-8'>
                        <ul>
                            <li>1.<span className='px-1'></span>Questions 1-10 will test your verbal reasoning .</li>
                            <li>2.<span className='px-1'></span>Questions 11-20 will test your quantitative ability.</li>
            ...
            <button onClick={handleSubmit} disabled={!beginStatus} title="Take Test"
